...
class UnscentedKalmanFilter(object):
    ...
    def update(self, z, R=None, UT=None, hx=None, **hx_args):
        ...
        **hx_args : keyword argument
        ...
        # zp는 unscent transform의 결과로 나온 추정값, self.S는 오차 공분산
        ...
        # residual 잔차 -> error : z는 실제로 들어온 값      
        self.y = self.residual_z(z, zp)   

        delta = 0.1
        abs_residual = np.abs(self.y)
        # hubber_loss_residual = np.where(abs_residual <= delta,
        #                                 0.5*self.y**2,
        #                                 delta*(abs_residual - 0.5*delta))        
        

        ## make the residual extrimely
        hubber_loss_residual = self.huber_loss(abs_residual, delta)
        print("hubber : ", hubber_loss_residual, "self.y : ", self.y)
        # update Gaussian state estimate (x, P)
        # self.x = self.state_add(self.x, dot(self.K, self.y))
        # self.x = self.state_add(self.x, dot(self.K, self.y)) #추정값 계산 part
        if self.y < 0:
            self.x = self.state_add(self.x, dot(self.K, -hubber_loss_residual)) #추정값 계산 part
        else:
            self.x = self.state_add(self.x, dot(self.K, hubber_loss_residual)) #추정값 계산 part
        self.P = self.P - dot(self.K, dot(self.S, self.K.T)) # 오차 공분산 계산
    ...
    ###재준
    def huber_loss(self, residual, delta):
